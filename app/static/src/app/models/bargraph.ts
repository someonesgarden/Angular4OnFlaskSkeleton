...
export class BarGraph {
  ...
  init(): void {
    // Bar Graph Init
    this.bar_dom = d3.select('#nobel-bar');
    let bar_margin = {top:10, right:10, bottom:10, left:10};
    let boundingRect = this.bar_dom.node().getBoundingClientRect();
    ...
    this.svg = this.bar_dom.append("svg")
  ...
  updateBarChart(data) {
    console.log('updateBarCHart');
        ...
        this.xScale.domain( data.map(function(d) { return d.code; }) );
        this.yScale.domain([0, d3.max(data, function(d: any) { return +d.value; })]);
        ...
        bars.classed('active', function(d) {
            ...
            .transition().duration(G.nbviz.TRANS_DURATION)
            .attr("x", (d)=> { return this.xScale(d.code); })
